...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Deploy Aliengo combined TorchScript policy (obs48+alpha12)")
    parser.add_argument("--policy-path", required=True)
    parser.add_argument("--alpha-source", choices=["zero", "fixed", "online-file"], default="zero")
    parser.add_argument("--alpha-path", default=None)
    parser.add_argument("--alpha-dim", type=int, default=12)
...
def build_observation(
    ...
    if obs.shape != (1, OBS_DIM):
        raise RuntimeError(f"obs48 shape mismatch: got {tuple(obs.shape)}, expected {(1, OBS_DIM)}")
...
def _normalize_alpha_tensor(alpha_tensor: Any, alpha_dim: int, source_name: str) -> torch.Tensor:
    alpha = torch.as_tensor(alpha_tensor, dtype=torch.float32)

    if alpha.ndim == 0:
        raise RuntimeError(f"Invalid alpha from {source_name}: scalar provided, expected length {alpha_dim}.")
    if alpha.ndim == 1:
        if alpha.numel() != alpha_dim:
            raise RuntimeError(
                f"Invalid alpha from {source_name}: shape {tuple(alpha.shape)}, expected ({alpha_dim},)."
            )
        return alpha.view(1, -1)

    if alpha.ndim == 2:
        if alpha.shape[1] != alpha_dim:
            raise RuntimeError(
                f"Invalid alpha from {source_name}: shape {tuple(alpha.shape)}, expected [N, {alpha_dim}]."
            )
        # Accept [1, D] directly or average multi-row alpha sets.
        if alpha.shape[0] == 1:
            return alpha
        return alpha.mean(dim=0, keepdim=True)

    alpha_flat = alpha.view(-1)
    if alpha_flat.numel() != alpha_dim:
        raise RuntimeError(
            f"Invalid alpha from {source_name}: shape {tuple(alpha.shape)}, expected total size {alpha_dim}."
        )
    return alpha_flat.view(1, -1)


def _extract_alpha_payload(obj: Any) -> Any:
    if isinstance(obj, dict):
        for key in ["alpha", "alpha_current"]:
            if key in obj:
                return obj[key]
    return obj


def _load_alpha_from_path(alpha_path: str, alpha_dim: int) -> torch.Tensor:
    if not os.path.isfile(alpha_path):
        raise RuntimeError(f"Alpha file not found: {alpha_path}")

    suffix = os.path.splitext(alpha_path)[1].lower()

    if suffix == ".json":
        with open(alpha_path, "r", encoding="utf-8") as f:
            payload = json.load(f)
        payload = _extract_alpha_payload(payload)
        return _normalize_alpha_tensor(payload, alpha_dim, alpha_path)

    if suffix == ".npy":
        try:
            import numpy as np  # type: ignore
        except Exception as import_err:
            raise RuntimeError(f"Cannot load .npy alpha without numpy: {import_err}")
        payload = np.load(alpha_path)
        return _normalize_alpha_tensor(payload, alpha_dim, alpha_path)

    # Treat as torch payload for .pt/.pth and unknown binary tensors.
    payload = torch.load(alpha_path, map_location="cpu")
    payload = _extract_alpha_payload(payload)
    return _normalize_alpha_tensor(payload, alpha_dim, alpha_path)


def _discover_online_alpha_path(policy_path: str, alpha_path: Optional[str]) -> Optional[str]:
    if alpha_path is not None and str(alpha_path).strip() != "":
        return alpha_path

    policy_dir = os.path.dirname(policy_path)
    parent_dir = os.path.dirname(policy_dir)

    candidates = [
        os.path.join(policy_dir, "online_alpha.pt"),
        os.path.join(parent_dir, "online_alpha.pt"),
        os.path.join(parent_dir, "alpha.pt"),
    ]

    # Common adapted package layout: <pkg>/policy_jit/policy_1.pt and <pkg>/online_alpha.pt
    if os.path.basename(policy_dir) == "policy_jit":
        candidates.append(os.path.join(parent_dir, "online_alpha.pt"))

    # Last fallback: check policy root sibling folder.
    grand_parent = os.path.dirname(parent_dir)
    candidates.append(os.path.join(grand_parent, "online_alpha.pt"))
...
def build_alpha(alpha_source: str, alpha_path: Optional[str], alpha_dim: int, policy_path: str) -> Tuple[torch.Tensor, str]:
    if alpha_source == "zero":
        alpha = torch.zeros((1, alpha_dim), dtype=torch.float32)
        return alpha, "zero"

    if alpha_source == "fixed":
        if alpha_path is None or str(alpha_path).strip() == "":
            raise RuntimeError("--alpha-source fixed requires --alpha-path <file>.")
        alpha = _load_alpha_from_path(alpha_path, alpha_dim)
        return alpha, f"fixed:{os.path.abspath(alpha_path)}"

    if alpha_source == "online-file":
        resolved = _discover_online_alpha_path(policy_path=policy_path, alpha_path=alpha_path)
        if resolved is None:
            raise RuntimeError(
                "--alpha-source online-file requested, but no online alpha file was found. "
                "Provide --alpha-path <online_alpha.pt> or use an adapted package containing online_alpha.pt."
            )
        alpha = _load_alpha_from_path(resolved, alpha_dim)
        return alpha, f"online-file:{os.path.abspath(resolved)}"

    raise RuntimeError(f"Unknown alpha source: {alpha_source}")
...
def main() -> int:
    ...
    if args.print_every <= 0:
        ...
        return 2
    if args.alpha_dim <= 0:
        print("ERROR: --alpha-dim must be > 0")
        return 2
    ...
    print("=" * 80)
    print("Aliengo deploy_combined_policy.py")
    print(f"policy_path={args.policy_path}")
    print(f"alpha_source={args.alpha_source} alpha_dim={args.alpha_dim} alpha_path={args.alpha_path}")
    print(
    ...
    print("UPESI online adaptation on real robot is NOT implemented in this script (safe inference only).")
    if args.alpha_source == "zero":
        print("WARNING: alpha_source=zero is only a safe fallback and may not match trained adaptation.")
    ...
    try:
        ...
        print(f"ERROR: failed to load TorchScript policy: {e}")
        return 3

    try:
        alpha, alpha_label = build_alpha(
            alpha_source=args.alpha_source,
            alpha_path=args.alpha_path,
            alpha_dim=args.alpha_dim,
            policy_path=args.policy_path,
        )
        print(f"Resolved alpha source: {alpha_label}")
    except Exception as e:
        print(f"ERROR: failed to build alpha: {e}")
        print("Hint: use --alpha-source zero for fallback, or provide valid --alpha-path for fixed/online-file.")
        return 3

    combined_obs_dim = OBS_DIM + int(args.alpha_dim)
    test_obs = torch.zeros((1, combined_obs_dim), dtype=torch.float32)
    test_combined = torch.cat([test_obs[:, :OBS_DIM], alpha], dim=-1)
    try:
        with torch.no_grad():
            test_action = policy(test_combined)
        _ = extract_action(test_action)
    except Exception as e:
        print("ERROR: policy forward failed for combined input.")
        print(f"  combined_obs shape tried: {tuple(test_combined.shape)}")
        print(f"  details: {e}")
        print(
            "This policy may not be an aliengo_combined actor expecting obs48+alpha12, "
            "or alpha dim/source is wrong."
        )
        return 3
    ...
    try:
        while running:
            ...
            combined_obs = torch.cat([obs48, alpha], dim=-1)
            if obs48.shape != (1, OBS_DIM):
                raise RuntimeError(f"obs48 shape mismatch: got {tuple(obs48.shape)}")
            if alpha.shape != (1, args.alpha_dim):
                raise RuntimeError(f"alpha shape mismatch: got {tuple(alpha.shape)}")
            if combined_obs.shape != (1, combined_obs_dim):
                raise RuntimeError(
                    f"combined_obs shape mismatch: got {tuple(combined_obs.shape)}, expected {(1, combined_obs_dim)}"
                )

            try:
                with torch.no_grad():
                    policy_out = policy(combined_obs)
            except Exception as e:
                print("ERROR: policy forward failed during loop.")
                print(f"  combined_obs shape: {tuple(combined_obs.shape)}")
                print(f"  details: {e}")
                print(
                    "This policy may not be an aliengo_combined actor expecting obs48+alpha12, "
                    "or alpha dim/source is wrong."
            ...
            if mode == "PREPARE":
                ...
                if stand_active:
                    ...
                    fill_lowcmd(
                        low_cmd,
                        current_q_sdk=current_q_sdk,
                        target_q_sdk=stand_target_q_sdk,
                        kp=args.kp,
                        kd=args.kd,
                    )
            ...
            if step_idx % int(args.print_every) == 0:
                ...
                send_now = False
                stand_send_now = False
                policy_send_now = False
                if mode == "PREPARE":
                    stand_send_now = bool(args.send_stand) and ((time.time() - prepare_start) <= float(args.stand_duration))
                    send_now = stand_send_now
                elif mode == "POLICY":
                    policy_send_now = send_policy_enabled
                    send_now = policy_send_now
                ...
                alpha_mean = float(torch.mean(alpha).item())
                blv = diag["base_lin_vel"]
                print(
                    f"[{mode}] step={step_idx} obs={tuple(obs48.shape)} combined_obs={tuple(combined_obs.shape)} "
                    f"alpha_source={args.alpha_source} alpha[min,max,mean]=({alpha_min:.3f},{alpha_max:.3f},{alpha_mean:.3f}) "
                    f"cmd=({args.vx:.2f},{args.vy:.2f},{args.yaw_rate:.2f}) "
                    f"a[min,max]=({action_min:.3f},{action_max:.3f}) "
                    f"base_lin_vel=({float(blv[0]):.3f},{float(blv[1]):.3f},{float(blv[2]):.3f}) "
                    f"roll={roll:.3f} pitch={pitch:.3f} send={send_now} stand_send={stand_send_now} policy_send={policy_send_now} "
    ...
    print("deploy_combined_policy finished.")
    return 0
